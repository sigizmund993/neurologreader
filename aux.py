...
class Point:
    ...
    def debug_str(self) -> str:
        """Returns a string to generate the same point"""
...
def dist2line(point1: Point, point2: Point, point: Point) -> float:
    """
    Рассчитать расстояние от точки p до прямой, образованной точками p1 и p2
    """
    return abs(vec_mult((point2 - point1).unity(), point - point1))
...
def nearest_point_on_poly(p: Point, poly: list[Point]) -> Point:
    """
    Получить ближайшую точку на краю полигона
    """
    min_ = 10e10
    ans = Point(0, 0)
...
def nearest_point_in_poly(p: Point, poly: list[Point]) -> Point:
    """
    Получить ближайшую точку на полигоне
    """
    if is_point_inside_poly(p, poly):
        return p

    return nearest_point_on_poly(p, poly)
...
def get_angle_between_points(a: Point, b: Point, c: Point) -> float:
    ...
    ang > 0  => c - слева, b - справа (относительно)
    """
...
def line_circle_intersect(x1: Point, x2: Point, c: Point, radius: float, is_inf: str = "L") -> list[Point]:
    """
    is_inf задает ограничения на точку пересечения.
    ...
    L(ine) - задана прямая
    """
    h = closest_point_on_line(x1, x2, c, is_inf)
    if radius < dist(c, h):
        return []
    if radius == dist(c, h):
        return [h]
...
def offset_polygon(peaks: list[Point], distance: float) -> list[Point]:
    """Offset edges of CONVEX polygons WITHOUT STRAIGHT ANGLES"""
    if len(peaks) < 3:
        return peaks
